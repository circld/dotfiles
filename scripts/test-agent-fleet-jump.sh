...
case "${1:-}" in
  ...
  action)
    ...
    if [ "${2:-}" = "list-panes" ]; then
      cat "$AGENT_FLEET_TEST_FAKE_DIR/panes-$session.json" 2>/dev/null || printf '[]\n'
    fi
    # Failure injection for the landing-ledger rc!=0 path (off by default).
    if [ "${2:-}" = "focus-pane-id" ] && [ -n "${AGENT_FLEET_TEST_FAIL_FOCUS:-}" ]; then
      exit 1
    fi ;;
esac
exit 0
